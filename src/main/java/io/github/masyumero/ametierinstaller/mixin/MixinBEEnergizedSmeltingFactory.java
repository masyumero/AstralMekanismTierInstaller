...
@Mixin(value = BEEnergizedSmeltingFactory.class, remap = false)
public abstract class MixinBEEnergizedSmeltingFactory extends BlockEntityProgressFactory<SmeltingRecipe, BEEnergizedSmeltingFactory> {
    ...
    protected MixinBEEnergizedSmeltingFactory(IBlockProvider blockProvider, BlockPos pos, BlockState state, int baseTicksRequired, List<CachedRecipe.OperationTracker.RecipeError> errorTypes, Set<CachedRecipe.OperationTracker.RecipeError> globalErrorTypes) {
        super(blockProvider, pos, state, baseTicksRequired, errorTypes, globalErrorTypes);
    }
    ...
    public SmeltingUpgradeData getUpgradeData() {
        return new SmeltingUpgradeData(redstone, getControlType(), getEnergyContainer(), progress, gasMode, energySlot, infusionTank, List.of(inputSlots), List.of(outputSlots), isSorting(), getComponents());
    }
}
